function totals_realtime(system_type, database_session, varargin)
%
...
%   'version'   - ID number for version of the totals we are processing
...
db_query = ['select hfrSites.site, hfrSites.id,',...
    'hfrLatestRadials.Timestamp, hfrFileTypes.type from hfrSites ',...
    'inner join hfrFileTypes on (hfrSites.radialProcessingType = hfrFileTypes.id) ',...
    'inner join hfrLatestRadials on (hfrSites.id = hfrLatestRadials.siteId) ',...
    'inner join hfrSystemTypes on (hfrSystemTypes.id = hfrSites.type) ',...
    'where (hfrSites.region = ' num2str(region) ' AND ',...
    'hfrSites.type = ' num2str(system_type) ' AND ',...
    'hfrSites.active = 1)'];
